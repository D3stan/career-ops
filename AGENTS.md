...
| `title-sieve.mjs` | Title-only LLM first pass over recent scan results: reads only `modes/_brief.md` + `title / company / location`, ~60 titles per CLI call, verdict `keep` / `unsure` / `drop` + short reason into `data/title-sieve.tsv`. A drop is recorded in `scan-history.tsv` as `skipped_sieve`, moved out of Pending in `pipeline.md` as a `#-- … skipped (title sieve: …)` discard, and logged to `data/discard.log`; `--restore <url>` reverses it. Also driven from the web Explore view ("Sieve titles") |
