...
| `data/title-sieve.tsv` | Title-sieve verdicts (`keep`/`unsure`/`drop`/`restored`), append-only, latest row per URL wins (written by `title-sieve.mjs`; drops are also recorded in `scan-history.tsv` as `skipped_sieve`) |
